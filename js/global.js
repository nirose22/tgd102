...
class HeaderDrawer extends MenuDrawer {
  constructor() {
    ...
    this.iconSearch = document.querySelector('.js-headerSearch summary')
    this.iconSearch.addEventListener('click', (event) => {
      setTimeout(() => {
  ...
  openMenuDrawer(summaryElement) {
    ...
    if ( window.innerWidth >= 750 ) {
      this.detailsMenu = document.querySelectorAll('[id^="Details-menu-drawer-menu-item"]');
      this.detailsMenu.forEach((details) => {
        details.open = true;
        details.setAttribute('open', '');
        details.classList.add('menu-opening');
      });
      this.detailsMenuChild = document.querySelectorAll('[id^="Details-menu-drawer-submenu"]');
      this.detailsMenuChild.forEach((details) => {
        details.open = true;
        details.setAttribute('open', '');
        details.classList.add('menu-opening');
      });
...
class EmailAddress extends HTMLElement {
  constructor() {
    super();
    const replaceStr = '__at__';
    let address = this.textContent;
    address = address.replace(replaceStr, '@');
    let aTag = '<a class="full-unstyled-link-inline" href="mailto:' + address + '" target="_blank">' + address + '</a>'
    this.innerHTML = aTag
  }
}
